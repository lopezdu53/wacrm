...
export function InboxWorkspace({ lane }: { lane: InboxLane }) {
  const t = useTranslations("Inbox.page");
  const router = useRouter();
  const basePath = lane === "opportunity" ? "/opportunities" : "/inbox";
  ...
  const hydrateConversation = useCallback(async (convId: string) => {
    ...
    try {
      ...
      if (!matchesInboxLane(fetched, lane)) {
        if (deepLinkConvId === fetched.id) {
          router.replace(inboxPathForConversation(fetched), { scroll: false });
        }
        return;
      }
      ...
      if (deepLinkConvId === fetched.id) {
        setActiveConversation((prev) => {
          if (prev?.id === fetched.id) {
            return { ...prev, contact: prev.contact ?? fetched.contact };
          }
          if (prev) return prev;
          return fetched;
        });
        setActiveContact((prev) => prev ?? fetched.contact ?? null);
      }
    } finally {
      hydratingConvIdsRef.current.delete(convId);
    }
  }, [deepLinkConvId, lane, router]);
  ...
  useEffect(() => {
    const checkConnection = async () => {
      ...
      // An account can now have multiple configs (migration 039), so
  ...
  const handleConversationEvent = useCallback(
    (event: {
      ...
      if (event.eventType === "INSERT") {
        ...
        if (!matchesInboxLane(conv, lane)) return;
    ...
    [activeConversation, basePath, hydrateConversation, lane, router]
  );
  ...
  const { isConnected } = useRealtime({
    channelName: lane === "opportunity" ? "opportunity-realtime" : "inbox-realtime",
  ...
  const handleOpportunityChange = useCallback(
    (conversationId: string, _isOpportunity: boolean) => {
  ...
  useEffect(() => {
    const unread = sumUnread(
      conversations.map((c) => Number(c.unread_count) || 0),
    );
    if (lane === "opportunity") {
      setOppUnread(unread);
      return () => setOppUnread(0);
    }
    setInboxUnread(unread);
    return () => setInboxUnread(0);
  }, [conversations, lane, setInboxUnread, setOppUnread]);
